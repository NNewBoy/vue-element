<template>
  <div class="app-container">
    ...
    <PlTable
      ...
      class="pl-table-edit tb-edit"
      @table-body-scroll="tableBodyScroll"
      @selection-change="handleSelectionChange"
      @cell-click="onClickTb"
    >
      <PlTableColumn type="selection" width="45" />
      <PlTableColumn align="center" label="ID" width="50">
        <template slot-scope="scope">{{ scope.$index + 1 }}</template>
      </PlTableColumn>
      <PlTableColumn label="删除" align="center" width="82" class-name="mini-padding fixed-width">
...
import CommonColumn from '@/components/MaterialEdit/column/PlCommonColumn'
import SelectColumn from '@/components/MaterialEdit/column/PlSelectColumn'
import SwitchColumn from '@/components/MaterialEdit/column/PlSwitchColumn'
import ButtonColumn from '@/components/MaterialEdit/column/PlButtonColumn'
...
export default {
  ...
  components: { PlTable, PlTableColumn, LazyFilter, CommonColumn, SelectColumn, SwitchColumn, ButtonColumn, BrandsFilter, StatusFilter, Collapse, CatalogLazyDialog, SelectDialog /*, Pagination*/ },
  filters: {
  ...
  methods: {
    ...
    tableRowStyle({ row, rowIndex }) {
      return row.changed === 1 ? { 'background-color': 'PeachPuff', 'height': this.rowHeight + 'px' } : { 'height': this.rowHeight + 'px' }
    },
    selectStatus(res) {
    ...
    onDelete(row, index) {
      editDelete(() => {
        ...
        deleteProduct(row.id).then(response => {
          ...
          this.lastScroll.updated = 'delete'
          this.loadingList = false
        }).catch(() => {
          this.loadingList = false
        })
    ...
    tableBodyScroll({ scrollTop, scrollLeft, table }) { // pl-table表体滚动时触发
      if (this.lastScroll.updated === 'next') {
        // 滚动到刷新前的位置，因为pl-table刷新数据时默认滚动到顶部
        this.lastScroll.updated = false
        this.$refs.tbTable.pagingScrollTopLeft(this.lastScroll.top + this.rowHeight, this.lastScroll.left)
      } else if (this.lastScroll.updated === 'delete') {
        this.lastScroll.updated = false
        this.$refs.tbTable.pagingScrollTopLeft(this.lastScroll.top, this.lastScroll.left)
