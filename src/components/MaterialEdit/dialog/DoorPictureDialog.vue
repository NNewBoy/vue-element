  ...
  <el-dialog :key="key" v-el-drag-dialog class="dialog-container" top="10vh" title="修改贴图属性" :visible.sync="visible">
    <div slot="title" class="dialog-footer">
      <div class="selected-condition">
        ...
        >
          <i class="el-icon-check" />{{ item['door_color'] }}
        </a>
      </div>
    </div>
 ...
 * @function returnDoorPicture 返回与tableData同类型的数组，对象包括id和修改的参数
 */
  ...
  methods: {
    ...
    initData() {
      ...
      if (this.materialCheckboxVal.length === 1) {
        const { door_color, folder, pic_file_texture_dir } = this.materialCheckboxVal[0]
